...
## 前端核心交互特色 (UI/UX Highlights)
1. **童趣化 ReAct 智能助手 (K)**：Agent 工具调用链路以拟人化生动徽章呈现（如“🔍 在故事百宝箱中寻找线索”、“✨ 挥动魔法棒构思精彩故事”），支持点击展开调用参数与执行结果抽屉；引用故事卡片支持点击浮层快速预览与一键直达详情。
2. **实时流式创作与插图异步化 (C)**：故事创作接通原生 SSE 流式输出与光标打字呼吸动效，并提供快速创意灵感 Chips；插图采用骨架屏占位并在后台异步生成后平滑淡入。
3. **音字同步指读与舒适阅读器 (T)**：详情页听书接入 Web Speech 逐句高亮（卡拉OK式指读体验），并提供大/中/小护眼字号调节、纯文本导出与打印样式排版优化。
4. **多元音色与规范化元数据 (G)**：后端统一定义多角色儿童音色（标准旁白、温柔姐姐、博学树爷爷、活泼小萌兔）并收敛全站故事分类及全局 Emoji 映射。
